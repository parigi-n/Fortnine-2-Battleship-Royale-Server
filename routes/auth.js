...
module.exports = [
  {
    ...
    handler: async (request, h) => {
      try {
        ...
        request.server.app.asyncRedisClient.setex(
          `${tokenData.userId}:${sessionId}`,
          expiresIn,
  ...
  },
  {
    ...
    handler: async (request) => {
      try {
        request.server.app.asyncRedisClient.del(
          `${request.auth.credentials.userId}:${request.auth.credentials.jti}`,
        );
        return {
          success: true,
        };
