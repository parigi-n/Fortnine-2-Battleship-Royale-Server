...
module.exports = [{
  ...
  handler: async (request, h) => {
    try {
      ...
      if (!response || response.length === 0) { return Boom.unauthorized('Invalid login'); }
  ...
  handler: async (request) => {
    try {
      ...
      if (responseCheckIfExist) { return Boom.conflict('An user already exists with this username'); }
